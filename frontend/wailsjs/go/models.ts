...
export namespace models {
	...
	export class FlatNode {
	    ...
	    constructor(source: any = {}) {
	        ...
	        this.parentIdx = source["parentIdx"];
	        this.hasChildren = source["hasChildren"];
	    }
	}
	export class FlatCatalog {
	    nodes: FlatNode[];
	    fileCount: number;
	    totalBytes: number;
	
	    static createFrom(source: any = {}) {
	        return new FlatCatalog(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.nodes = this.convertValues(source["nodes"], FlatNode);
	        this.fileCount = source["fileCount"];
	        this.totalBytes = source["totalBytes"];
	    }
	...
	export class SearchIndexResult {
	    results: SearchResult[];
	    total: number;
	
	    static createFrom(source: any = {}) {
	        return new SearchIndexResult(source);
	    }
