...
function CreateCatalogSidebar() {
  ...
  const createCatalog = async () => {
    ...
    try {
      const result = await window.electronAPI.createCatalog(
        catalogTitle.trim(),
        state.selectedDirectory,
        outputRoot.trim(),
        copyToSecondary ? (state.selectedOutputDirectory || '') : ''
      );

      if (result.success) {
        message.success(`Catalog created successfully!`);
      } else {
        message.error(`Failed to create catalog: ${result.error || 'Unknown error'}`);
      }
    } catch (error) {
...
CreateCatalogTab.Sidebar = CreateCatalogSidebar;
CreateCatalogTab.Content = CreateCatalogContent;
