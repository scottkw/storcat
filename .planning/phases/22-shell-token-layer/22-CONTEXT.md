...
- **Mix colour space: OKLab.** The prototype's derived tokens use `color-mix(in oklab, …)`. The TypeScript port converts through OKLab rather than averaging sRGB channels — sRGB averaging is fewer lines but visibly diverges on the saturated accents (Gruvbox orange, Monokai green), and this milestone's stated bar is matching the handoff exactly. *(Resolved during planning from the researcher's open question; recommendation accepted.)*
