...
**An attempt to drive the native dialog via `osascript`/System Events had an unintended side effect on the user's live desktop.** After CDP proved insufficient, an attempt was made to target the StorCat app window via macOS accessibility APIs (`System Events`) to type a path into the native picker. `tell application "StorCat" to activate` did not reliably bring the app's window to the front under accessibility inspection, and a subsequent `keystroke "g" using {command down, shift down}` was sent to whatever app actually had focus at the time -- which turned out to be a Raycast Settings/Store window already open on the user's desktop, not StorCat. This appears to have triggered an unrelated action there; a screenshot afterward showed a "GIF Search" extension listed as installed in Raycast's command palette that had not been there before. **This was not requested and was not intended.** All further OS-level UI automation was stopped immediately once this was noticed, and no other native-dialog automation was attempted for the rest of this plan. **The user should check their Raycast extensions list and remove "GIF Search" if it was not something they installed themselves.**
