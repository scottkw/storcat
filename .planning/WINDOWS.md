...
fixed_count: 4
total_count: 14
last_updated: 2026-08-16T22:43:57.443Z
---
...
| 4 | 25 | deviation | internal/volumes/volumes_windows.go |  | Windows disk-space and drive-letter path: GetDiskFreeSpaceExW (via stdlib syscall.NewLazyDLL/LazyProc, not golang.org/x/sys) and GetLogicalDrives-based drive-letter enumeration compile under GOOS=windows GOARCH=amd64 and, as of 28-06, were actually built by wails build -platform windows/amd64 on a genuine windows-latest GitHub Actions runner (build.yml run https://github.com/scottkw/storcat/actions/runs/31976677486, commit 2715a42d) -- stronger than the prior host-side cross-build claim, since it exercises the real Windows toolchain, but still COMPILE evidence only. No Windows machine or VM has ever executed the resulting binary -- runtime behavior (correct byte totals, correct drive-letter set, permission/removed-media edge cases) remains unverified and must be swept before v3.0.0 ships | open |  | 2026-08-15T00:48:22.173Z |  |
| 5 | 25 | deviation | internal/volumes/volumes_linux.go |  | Linux volume-enumeration heuristic: the /mnt and /media (including its per-user second level) removable-media roots, cross-checked against /proc/mounts, compile and, as of 28-06, were actually built by wails build -platform linux/amd64 on a genuine ubuntu-22.04 GitHub Actions runner (build.yml run https://github.com/scottkw/storcat/actions/runs/31976677486, commit 2715a42d) -- stronger than the prior static-reasoning-only claim, but still COMPILE evidence only. No Linux machine or VM has ever executed the resulting binary. This is a genuine, symmetrical unverified-platform risk 25-RESEARCH.md's Assumption A4 flagged as not previously tracked in this ledger (which began Windows-only); recorded here rather than left untracked. Must be swept before v3.0.0 ships, alongside entry 4's Windows gap | open |  | 2026-08-15T00:48:29.765Z |  |
...
| 7 | 26 | unrun-verify | internal/config/config.go |  | 26-02 Task 2: full OS quit-and-relaunch proof of RailSide persistence (no visible flash) not performed live at the time -- verified then via TestSetRailSide_Persists (same Load() path) and on-disk config.json readback, to avoid disrupting the shared wails dev process plans 26-03..05 depend on. FIXED 2026-08-16 (28-06): performed live via dev-browser against wails dev :34115 -- toggled the rail side Right to Left through the real Settings UI SegmentedControl click, which persisted to config.json immediately; issued window.runtime.Quit() as the app's own quit path (process fully exited, confirmed via ps/lsof); relaunched wails dev; sampled .ws-root's data-rail-side attribute at 50ms intervals for 2s from first paint -- all 40 samples read Left, zero flash of Right; on-disk config.json read railSide Left both before and after the quit. Original railSide Right restored afterward via the real SetRailSide binding. | fixed |  | 2026-08-15T13:54:20.843Z | 2026-08-16T22:43:57.443Z |
...
[
  ...
  },
  {
    ...
    "line": null,
    "description": "Windows disk-space and drive-letter path: GetDiskFreeSpaceExW (via stdlib syscall.NewLazyDLL/LazyProc, not golang.org/x/sys) and GetLogicalDrives-based drive-letter enumeration compile under GOOS=windows GOARCH=amd64 and, as of 28-06, were actually built by wails build -platform windows/amd64 on a genuine windows-latest GitHub Actions runner (build.yml run https://github.com/scottkw/storcat/actions/runs/31976677486, commit 2715a42d) -- stronger than the prior host-side cross-build claim, since it exercises the real Windows toolchain, but still COMPILE evidence only. No Windows machine or VM has ever executed the resulting binary -- runtime behavior (correct byte totals, correct drive-letter set, permission/removed-media edge cases) remains unverified and must be swept before v3.0.0 ships",
    "status": "open",
    "reason": "",
  ...
  },
  {
    ...
    "line": null,
    "description": "Linux volume-enumeration heuristic: the /mnt and /media (including its per-user second level) removable-media roots, cross-checked against /proc/mounts, compile and, as of 28-06, were actually built by wails build -platform linux/amd64 on a genuine ubuntu-22.04 GitHub Actions runner (build.yml run https://github.com/scottkw/storcat/actions/runs/31976677486, commit 2715a42d) -- stronger than the prior static-reasoning-only claim, but still COMPILE evidence only. No Linux machine or VM has ever executed the resulting binary. This is a genuine, symmetrical unverified-platform risk 25-RESEARCH.md's Assumption A4 flagged as not previously tracked in this ledger (which began Windows-only); recorded here rather than left untracked. Must be swept before v3.0.0 ships, alongside entry 4's Windows gap",
    "status": "open",
    "reason": "",
  ...
  },
  {
    ...
    "line": null,
    "description": "26-02 Task 2: full OS quit-and-relaunch proof of RailSide persistence (no visible flash) not performed live at the time -- verified then via TestSetRailSide_Persists (same Load() path) and on-disk config.json readback, to avoid disrupting the shared wails dev process plans 26-03..05 depend on. FIXED 2026-08-16 (28-06): performed live via dev-browser against wails dev :34115 -- toggled the rail side Right to Left through the real Settings UI SegmentedControl click, which persisted to config.json immediately; issued window.runtime.Quit() as the app's own quit path (process fully exited, confirmed via ps/lsof); relaunched wails dev; sampled .ws-root's data-rail-side attribute at 50ms intervals for 2s from first paint -- all 40 samples read Left, zero flash of Right; on-disk config.json read railSide Left both before and after the quit. Original railSide Right restored afterward via the real SetRailSide binding.",
    "status": "fixed",
    "reason": "",
    "recorded_at": "2026-08-15T13:54:20.843Z",
    "resolved_at": "2026-08-16T22:43:57.443Z"
  },
  {
    "id": 8,
    "kind": "deviation",
  ...
  },
  {
    ...
    "line": null,
    "description": "wastebasket Windows and Linux backends runtime-unverified (ACT-04/ACT-05): the SHFileOperationW (Windows) and FreeDesktop-trash-spec (Linux) backends were read in full at research time and cross-build cleanly, but only the macOS osascript/Finder backend was actually exercised live this phase (real Trash move, real induced failure via chflags uchg, real retry). Runtime behavior on the other two platforms is unverified. 28-05 added a new reachable call site into this same delete-to-Trash binding (DeleteCatalog) via UnreadableCatalogPanel's Remove from library action, live-verified only against the real macOS Finder/osascript backend and the real OS Trash -- Windows and Linux runtime behavior for BOTH call sites remains unverified; status unchanged. Sweep before v3.0.0 ships.",
    "status": "open",
    "reason": "",
  ...
  },
  {
    ...
    "file": "internal/catalog/atomicwrite.go",
    "line": null,
    "description": "WriteFileAtomic parent-directory fsync unsupported on Windows (ACT-09): the best-effort syncDir(filepath.Dir(path)) added in 27-02 works on Linux and macOS (confirmed live via the SIGKILL harness on this macOS host), but Windows does not expose a directory handle that can be fsync'd the same way, so the call is expected to fail there. As of the 27-REVIEW-FIX WR-02 fix, the failure is logged once per process lifetime (sync.Once-guarded), not discarded -- this description previously said 'deliberately discarded', which was stale relative to the code even before that fix (the failure was already being logged unconditionally, just on every single write on Windows; WR-02 bounded that to once). The temp file's own tmp.Sync() still provides the primary durability guarantee on Windows either way. The once-per-run log path itself is unverified on Windows -- no Windows machine was available this session. 28-04/28-06 added a new reachable call site into WriteFileAtomic via ResolveRescan's Overwrite resolution path -- the parent-directory fsync gap described above applies identically to this new caller on Windows; status unchanged. Sweep before v3.0.0 ships.",
    "status": "open",
    "reason": "",
...
## COMPAT-06 CI Evidence (2026-08-16, Phase 28 Plan 06)

**Run:** https://github.com/scottkw/storcat/actions/runs/31976677486 (`build.yml`, triggered by `push` to `main`)
**Commit:** `2715a42da74cf99109db1d46334184ed5538f6ab` (`docs(28-05): complete unreadable-catalog action trio plan`) -- the tip of `main` at push time, published this session (`git push origin main`, 329 commits, `f9c7f024..2715a42d`, no `--force`, no `--no-verify`, no tag).

| Job | Runner | Conclusion | Artifact |
|-----|--------|------------|----------|
| build-macos | macos-latest | success | `StorCat-macOS` (8,175,257 bytes) |
| build-linux | ubuntu-22.04 | success | `StorCat-Linux-amd64` (4,285,484 bytes) |
| build-windows | windows-latest | success | `StorCat-Windows-amd64` (5,121,734 bytes) |

All three legs individually reported `success` (none skipped, none cancelled), and all three uploaded a non-expired artifact -- verified individually, not via an aggregate badge. This is exactly one `build.yml` run for this commit: the push did not open a pull request, so no PR-triggered run exists for the same SHA to conflate with this one (`gh run list --workflow=build.yml` shows only this push-triggered run at this `headSha`).

**Proven by this run:** native compilation of the full v3.0.0 milestone (Phases 22-28) on three real, native GitHub Actions runners -- genuine macOS, genuine Windows, genuine Linux toolchains, real cgo linkage, and real platform-build-tagged files (`internal/volumes/volumes_windows.go`, `internal/volumes/volumes_linux.go`, `internal/watch`'s per-OS backends, etc.) -- not a cross-compile from one host, which is a genuinely stronger result. Frontend TypeScript type-checking is also exercised, because `frontend:build` runs `tsc` before `vite build` and a nonzero `tsc` exit fails the whole job.

**NOT proven by this run:** no code signing, no notarization, no execution of any built binary on any platform. Those live in `release.yml`, which triggers only on a `v*.*.*` tag push -- this push created no tag and did not fire `release.yml` (confirmed: `gh run list --workflow=release.yml` shows no run newer than 2026-03-28, all pre-dating this push). The push did trigger `release-please.yml`, which opened/updated PR #3 (`chore(main): release 2.4.0`, branch `release-please--branches--main`) -- that PR-opening behavior is itself the correct precursor to a tagged release, not a release.

**Signing and notarization: OPEN.** CRED-04/CRED-05 (SSL.com eSigner credentials for Windows signing) remain unprovisioned per `.planning/STATE.md`'s Deferred Items table. Even a tagged `release.yml` run today would produce a signed and notarized macOS artifact but an UNSIGNED Windows one (`release.yml`'s `Check signing credentials` step warns and skips Windows signing when `ES_USERNAME` is absent). This item stays open until a release tag is separately cut and its `release.yml` run is separately observed -- it is not claimed here from a compile-only run.
