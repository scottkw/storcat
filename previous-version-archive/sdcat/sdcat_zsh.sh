...
command_var=$1
command_var=${command_var:u}

# Check for COMMAND argument
[[ -z "$command_var" ]] && { echo "Missing command."; echo ""; usage; exit 1; }

if [ "$command_var" = "SEARCH" ] || [ "$command_var" = "S" ]; then
    [[ -z "$2" ]] && { echo "Missing search term."; echo ""; usage; exit 1; }
    searchCatalogs "$2"
    echo ""
    echo "---===<<< SEARCH COMPLETE >>>===---"
elif [ "$command_var" = "CREATE" ] || [ "$command_var" = "C" ]; then
